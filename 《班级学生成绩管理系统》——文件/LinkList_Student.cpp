...
{
	//打开文件
	FILE* pf = fopen("Students'data.txt", "r");
	if (pf == NULL)
	{
		printf("加载文件数据失败！\n");
		exit(1);
	...
	//创建临时变量储存文件中数据
	int tem_ID = 0; char tem_name[20] = { 0 }; double tem_score = 0;
	while (fscanf(pf, "%d %19s %lf", &tem_ID, tem_name, &tem_score) == 3)
	{
		//统计学生个数
		k++;
		//创建新节点
		struct LinkNode* oldStudent = (struct LinkNode*)malloc(sizeof(struct LinkNode));
		//给节点赋值
		oldStudent->ID = tem_ID;
		strcpy(oldStudent->name, tem_name);
		oldStudent->score = tem_score;
